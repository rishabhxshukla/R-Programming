...
summary(itemsets)
inspect(head(sort(itemssets, by="support"), 10))
itemsets <- apriori(Groceires, parameter = list(minlen=2, maxlen=2, support=0.02, target="frequent itemsets"))

summary(itemsets)
inspect(head(sort(itemssets, by="support"), 10))
itemsets <- apriori(Groceires, parameter = list(minlen=3, maxlen=3, support=0.02, target="frequent itemsets"))

summary(itemsets)
inspect(head(sort(itemssets, by="support"), 10))
itemsets <- apriori(Groceires, parameter = list(minlen=4, maxlen=4, support=0.02, target="frequent itemsets"))

summary(itemsets)
inspect(head(sort(items, by = "support"), 10))
